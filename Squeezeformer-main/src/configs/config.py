...
#pop :loại bỏ khỏi thuộc tính gốc.
class DatasetConfig:
...
# config.pop: lấy giá trị từ dict config bằng key, đồng thời xóa luôn key-value khởi dict.
...
class Config:
    ...
    def __init__(self, data: Union[str, dict]):
        #Nếu data là dict thì dùng luôn
        #Chuyển yaml về dạng dictionary
        config = data if isinstance(data, dict) else file_util.load_yaml(file_util.preprocess_paths(data))
        
        #Lấy giá trị tương ứng với speech_config trong config, nếu không tồn tại trả về {}
        # ,đồng thời xóa keycspeech_config khỏi config
