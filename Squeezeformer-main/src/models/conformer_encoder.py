...
class FFModule(tf.keras.layers.Layer):
    def __init__(
        ...
        **kwargs,
        ...
        # Tinsg giới hạn RandomUniform initializer giúp model khởi tạo ổn định hơn.
        ...
        #Giảm số chiều trở lại input_dim để khớp với residual.
...
# build được gọi tự động khi model nhận đầu vào lần đầu tiên
# call được gọi cho forward pass
    ...
    def __init__(
        ...
        **kwargs,
        ...
        encoder_subsampling:
            ...
            kernel_size: 3
            strides: 2"""
        #có type thì lấy rồi xóa khỏi dict, ko có lấy mặc định conv2d
        ...
        #Chuẩn hóa (khởi tạo) số chiều đặc trưng đầu vào
        #Tăng/ giảm số chiều về dmodel, chỉ thay đổi chiều cuối cùng dmodel = 144
    ...
    def call(self, inputs, length, training=False, mask=None, **kwargs):
        ...
        for i, cblock in enumerate(self.conformer_blocks):
            if self.time_reduce is not None and i in self.reduce_idx:
                ...
                # Tính lại Positional Encoding với chuỗi đã được nén.
                pe = self.pe_time_reduction[index](outputs)
                #Xử lý layer tiếp theo
                index += 1

            if self.time_reduce == 'recover' and i in self.recover_idx:
                # Giam index để dùng đúng recover activation đã lưu ở bước nén.
                index -= 1
