...
#Bộ trích xuất đặc trưng biến đổi văn bản thành chuỗi số nguyên (IDs) và ngược lại
# Dựa trên model SentencePiece- được pt bởi Google
# Chuyển văn bản -> IDs (số nguyên ) để đưa vào model huấn luyện
# Chuyển IDs -> văn bản: để decode output của model thành dạng đọc được
#Chuyển IDs -> Unicode code points: phục vụ TFLite.
class SentencePieceFeaturizer(TextFeaturizer):
    """
    Extract text feature based on sentence piece package.
    """
    UNK_TOKEN, UNK_TOKEN_ID = "<unk>", 1  #ko biết
    BOS_TOKEN, BOS_TOKEN_ID = "<s>", 2     # Bắt đầu câu
    EOS_TOKEN, EOS_TOKEN_ID = "</s>", 3     #Kết thúc câu
    ...
    #Chuyển văn bản thành list các ID
    ...
    #Chuyển list các ID thành mã Unicode (code points)
    # Để deploy mô hình với TFLite vì TFLite không hỗ trợ string, chỉ hỗ trợ int32.
